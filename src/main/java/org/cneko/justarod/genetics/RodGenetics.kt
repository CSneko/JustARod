...
object RodGenetics {
    // ========== 基因座 (6号染色体 - 长度) ==========
    val LENGTH_SLOT_0 = Locus(ResourceLocation.fromNamespaceAndPath(MODID, "length_slot_0"))
    val LENGTH_SLOT_1 = Locus(ResourceLocation.fromNamespaceAndPath(MODID, "length_slot_1"))
    val LENGTH_SLOT_2 = Locus(ResourceLocation.fromNamespaceAndPath(MODID, "length_slot_2"))

    // ========== 基因座 (7号染色体 - 宽度 & 高潮强度) ==========
    val WIDTH_SLOT_0 = Locus(ResourceLocation.fromNamespaceAndPath(MODID, "width_slot_0"))
    val WIDTH_SLOT_1 = Locus(ResourceLocation.fromNamespaceAndPath(MODID, "width_slot_1"))
    val ORGASM_SLOT_0 = Locus(ResourceLocation.fromNamespaceAndPath(MODID, "orgasm_slot_0"))
    val ORGASM_SLOT_1 = Locus(ResourceLocation.fromNamespaceAndPath(MODID, "orgasm_slot_1"))

    // ========== 等位基因 - 长度 (每人贡献一部分，多个可叠加) ==========

    // LENGTH_SLOT_0: +0.0 / +0.2 / +0.4
    val LENGTH_0_NORMAL = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "length_0_normal"), 10, null, null)
    val LENGTH_0_LONGER = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "length_0_longer"), 15,
        { _, data -> data.putDouble("length_0", 0.2) },
        { _, data -> data.remove("length_0") })
    val LENGTH_0_LONG = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "length_0_long"), 5,
        { _, data -> data.putDouble("length_0", 0.4) },
        { _, data -> data.remove("length_0") })

    // LENGTH_SLOT_1: +0.0 / +0.15 / +0.3
    val LENGTH_1_NORMAL = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "length_1_normal"), 10, null, null)
    val LENGTH_1_LONGER = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "length_1_longer"), 15,
        { _, data -> data.putDouble("length_1", 0.15) },
        { _, data -> data.remove("length_1") })
    val LENGTH_1_LONG = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "length_1_long"), 5,
        { _, data -> data.putDouble("length_1", 0.3) },
        { _, data -> data.remove("length_1") })

    // LENGTH_SLOT_2: +0.0 / +0.1 / +0.25
    val LENGTH_2_NORMAL = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "length_2_normal"), 10, null, null)
    val LENGTH_2_LONGER = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "length_2_longer"), 15,
        { _, data -> data.putDouble("length_2", 0.1) },
        { _, data -> data.remove("length_2") })
    val LENGTH_2_LONG = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "length_2_long"), 5,
    ...
    val WIDTH_0_NORMAL = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "width_0_normal"), 10, null, null)
    val WIDTH_0_WIDER = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "width_0_wider"), 15,
        { _, data -> data.putDouble("width_0", 0.2) },
        { _, data -> data.remove("width_0") })
    val WIDTH_0_THICK = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "width_0_thick"), 5,
        { _, data -> data.putDouble("width_0", 0.35) },
        { _, data -> data.remove("width_0") })

    // WIDTH_SLOT_1: +0.0 / +0.15 / +0.25
    val WIDTH_1_NORMAL = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "width_1_normal"), 10, null, null)
    val WIDTH_1_WIDER = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "width_1_wider"), 15,
        { _, data -> data.putDouble("width_1", 0.15) },
        { _, data -> data.remove("width_1") })
    val WIDTH_1_THICK = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "width_1_thick"), 5,
    ...
    val ORGASM_0_NORMAL = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "orgasm_0_normal"), 10, null, null)
    val ORGASM_0_STRONG = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "orgasm_0_strong"), 15,
        { _, data -> data.putDouble("orgasm_0", 0.5) },
        { _, data -> data.remove("orgasm_0") })
    val ORGASM_0_INTENSE = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "orgasm_0_intense"), 5,
        { _, data -> data.putDouble("orgasm_0", 1.0) },
        { _, data -> data.remove("orgasm_0") })

    // ORGASM_SLOT_1: 基础 +0.0 / +0.3 / +0.6
    val ORGASM_1_NORMAL = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "orgasm_1_normal"), 10, null, null)
    val ORGASM_1_SENSITIVE = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "orgasm_1_sensitive"), 15,
        { _, data -> data.putDouble("orgasm_1", 0.3) },
        { _, data -> data.remove("orgasm_1") })
    val ORGASM_1_HYPER = Allele(ResourceLocation.fromNamespaceAndPath(MODID, "orgasm_1_hyper"), 5,
    ...
    /** 计算总长度加成 */
    fun getTotalLengthBonus(data: CompoundTag): Double {
    ...
    /** 计算总宽度加成 */
    fun getTotalWidthBonus(data: CompoundTag): Double {
        return data.getDouble("width_0") +
               data.getDouble("width_1")
    }

    /** 计算高潮强度倍率 (基础 1.0 + 各基因贡献) */
    fun getOrgasmMultiplier(data: CompoundTag): Double {
    ...
    private fun addWild(locusId: ResourceLocation, alleleId: ResourceLocation, weight: Int) {
