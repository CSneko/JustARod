...
class SexualIntercourseGoal(private val neko: SeeeeexNekoEntity) : Goal() {
    // 将目标类型改为 INeko 以兼容 Player 和 NekoEntity
    ...
    fun slowTick() {
        ...
        if (world is ServerLevel) {
            ...
            if (neko.sexualDesire >= 50) {
                ...
                // 1. 优先寻找范围内的玩家 (默认行为)
                var newTarget: INeko? = neko.level().getPlayerInRange(neko, range).find { neko.canMate(it as INeko) } as INeko?

                // 2. 如果开启了AUTO_MATE_WITH_NEKO 且 周围没有合适的玩家，则寻找周围其他的Neko
                if (newTarget == null && SeeeeexNekoEntity.AUTO_MATE_WITH_NEKO) {
                    val searchBox = neko.boundingBox.inflate(range.toDouble())
                    newTarget = neko.level().getEntitiesOfClass(NekoEntity::class.java, searchBox) {
