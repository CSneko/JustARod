...
class HPVVaccine(properties: Properties) : MedicalItem(properties) {
    ...
    override fun getFailureMessage(user: Player, target: LivingEntity, stack: ItemStack): Component {
        if (target !is Pregnant) {
            return Component.literal("§c此物品只能对玩家使用。")
        }
    ...
    override fun getSuccessMessages(user: Player, target: LivingEntity, stack: ItemStack): ActionMessages {
        ...
        return ActionMessages(
            userSuccessMessage = if (isSelf) Component.literal("§a你接种了HPV疫苗，现在你已获得免疫力。") else Component.literal("§a你为 ${target.displayName?.string} 接种了HPV疫苗。"),
            targetSuccessMessage = if (isSelf) null else Component.literal("§a${user.displayName?.string} 为你接种了HPV疫苗，你获得了免疫力。")
        )
    }
}